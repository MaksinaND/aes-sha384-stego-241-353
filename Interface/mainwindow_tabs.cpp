...
MainWindow::MainWindow(QWidget *parent)
    ...
    , m_aesCipher(new AESCipher(this))
...
void MainWindow::onEncryptClicked() {
    ...
    // Устанавливаем ключ
    if (!m_aesCipher->setKey(key.toUtf8(), keySize)) {
        QMessageBox::critical(this, "Ошибка", "Не удалось установить ключ шифрования");
        return;
    }
    
    // Шифруем текст
    QByteArray cipherText = m_aesCipher->encrypt(plainText.toUtf8());
...
void MainWindow::onDecryptClicked() {
    ...
    // Дешифруем текст
    QByteArray plainText = m_aesCipher->decrypt(cipherText);
...
void MainWindow::onKeySizeChanged(int index) {
    // Обновляем информацию о размере ключа
    int keySize = 0;
    int keyBytes = 0;
    ...
    ui->labelKeySize->setText(QString("Размер ключа: %1 бит (%2 байт)").arg(keySize).arg(keyBytes));
}
